...
function enhanceDirectionalLight(mask, mv, groundTruth, initialVector, objectPosition, alpha, origOpacity, debug, vDebug, v3len, first = true)
{
	...
	if (false)//if (first)
	{
		...
		ctx2.fillText("initial",       580, 532 - 450 * ((mv - minVal) / (maxVal - minVal)));
		ctx2.fillText("est.",     580, 552 - 450 * ((mv - minVal) / (maxVal - minVal)));
/*
		var minImg = vl[0][0][1];
		var maxImg = vl[0][0][1];
		var minInt = vl[0][0][4];
...
*/
		if (false)//if (debug)
		{
			...
			link.setAttribute('download', (fc < 9 ? "0" : "") + (fc + 1).toString() + ".png");
...
//	}
	var newAlpha = alpha * 0.71167; // Math.sqrt((1 - Math.sqrt(Math.PI) / 4)^2 + Math.PI / 16)
	if (newAlpha >= 1.035 && res[0].angleTo(groundTruth) > initialVector.angleTo(groundTruth))
...
*/
	...
	return list[0]; // para na primeira recursao
	/*
	var sum = 0;
	for (var i = 0; i < 4; i++)
